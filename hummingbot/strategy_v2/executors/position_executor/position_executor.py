...
class PositionExecutor(ExecutorBase):
    ...
    async def control_close_order(self):
        ...
        if self._close_order:
            ...
            if in_flight_order:
                ...
                self._current_retries += 1
                self.logger().error(
                    f"Close order missing on connector {self.config.connector_name}, retrying "
                    f"({self._current_retries}/{self._max_retries})")
        else:
    ...
    def on_stop(self):
        if self.close_type == CloseType.EARLY_STOP:
            open_delay = f"open-delay={self.get_open_delay():.4f}s, " if self.get_open_delay() else "None, "
            open_slippage = f"open-slippage={self.get_open_slippage():.4%}, " if self.get_open_slippage() else "None, "
            close_delay = f"close-delay={self.get_close_delay():.4f}s, " if self.get_close_delay() else "None, "
            close_slippage = f"close-slippage={self.get_close_slippage():.4%}, " if self.get_close_slippage() else "None,"
            self.logger().info(f"Stopping position-executor, "
                            f"close-type={self.close_type.name}, " + open_delay +  open_slippage + close_delay + close_slippage + \
    ...
    def process_order_canceled_event(self, _, market: ConnectorBase, event: OrderCancelledEvent):
        ...
        if self._close_order and event.order_id == self._close_order.order_id:
            ...
            self._current_retries += 1
            self.logger().warning(
                f"Close order cancelled {event.order_id}. Retrying {self._current_retries}/{self._max_retries}")
        if self._open_order and event.order_id == self._open_order.order_id:
            ...
            self._current_retries += 1
            self.logger().warning(
                f"Open order cancelled {event.order_id}. Retrying {self._current_retries}/{self._max_retries}")
        if self._take_profit_limit_order and event.order_id == self._take_profit_limit_order.order_id:
            ...
            self._current_retries += 1
            self.logger().warning(
                f"Take profit order cancelled {event.order_id}. Retrying {self._current_retries}/{self._max_retries}")
